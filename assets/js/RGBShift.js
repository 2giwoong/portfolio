class RGBShift extends MainHoverEffect {
	constructor(container = document.body, effectWrapper, options = {}) {
		super(container, effectWrapper);
	...
	init() {
		...
		this.geometry = new THREE.PlaneBufferGeometry(0.7, 0.7, 32, 32);
	...
	onMouseEnter() {
		if (this.currentItem && this.currentItem.image) {
			gsap.to(this.uniforms.uAlpha, 0.1, {
				value: 1,
				ease: Power4.easeOut,
			});
		}
	...
	onTargetChange(index) {
		...
		// compute image ratio
		let imageRatio = this.currentItem.image.naturalWidth / this.currentItem.image.naturalHeight;
